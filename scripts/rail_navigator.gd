...
func _extend(front: bool) -> bool:
	...
	for value in choices:
		...
		var key := _edge_key(tip, cell)
		# Fresh branches win; thereafter balance use, preferring straight rails.
		var score := float(visits.get(key, 0)) * 10.0
		if cell - tip != tip - previous: score += 1.0
		if track.built_cells.has(cell): score -= 2.0
		if score < best_score:
			best_score = score
			best = cell
...
func ensure_tail(tail: float) -> bool:
	while distance < tail:
		if not _extend(false): return false
	return true

func valid(at: float, count: int, spacing: float, clearance: float) -> bool:
	if at < count * spacing - 0.01 or at > length() + 0.01: return false
...
func advance(amount: float, count: int, spacing: float, clearance: float) -> bool:
	...
	while remaining > 0.001:
		...
		if room < step:
			if _extend(forward): continue
			step = maxf(room, 0.0)
			buffer_hit = true
		var candidate := distance + step * (1.0 if forward else -1.0)
		if not valid(candidate, count, spacing, clearance): return false
		distance = candidate
		remaining -= step
		if buffer_hit: break
...
func _refresh_network(tail: float) -> void:
	if known_revision == track.revision: return
	...
	for index in range(points.size() - 1):
		...
		if b in track.graph.get(a, []): continue
		if index * track.path_step >= distance - 0.01:
			points.resize(index + 1)
			break
	var remove_count := 0
	for index in range(points.size() - 1):
		if (index + 1) * track.path_step > distance - tail + 0.01: break
		var a := track.cell_of(points[index])
		var b := track.cell_of(points[index + 1])
		if b not in track.graph.get(a, []): remove_count = index + 1
	if remove_count > 0:
