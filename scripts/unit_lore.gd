...
const ENTRIES := {
	...
	"Brake Van": "Caps off the train it is hooked up to, meaning no other cars can be added to that train once a Brake Van is attached.\n\nHowever, capping off the train grants all attacking units in it 25% more damage without changing their firing rate. It also slightly decreases the time it takes to start moving and to come to a stop.",
